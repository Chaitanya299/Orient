...
## Decision
No `flow.md` is maintained. `/bearings:trace` generates the execution path on demand and writes nothing to disk. `architecture.md` keeps only the durable skeleton — entry points, module boundaries, critical paths — the parts that change slowly enough to be worth writing down.
