...
## Context
This system has to be usable by someone who will not read a setup guide — the person who most needs a way to keep their orient is the one with the least patience for installing one. The distribution mechanism is part of the design, not an afterthought.

## Decision
Ship Orient as a plugin inside a marketplace repo, installed with two commands and updated through the same channel.
