...
## Decision
The block Bearings writes into `CLAUDE.md` stays at or under twenty lines and contains only pointers — names of files to read on demand when the work actually calls for them. The content lives in the files; only the pointers live in `CLAUDE.md`.
